...
const popupAvatar = new PopupWithForm(popupEditAvatar, handleSubmitFormAvatar);
...
const handleSubmitFormProfile = (userInfoData) => {
  ...
  api
    .updateProfileInfo(userInfoData.name, userInfoData.about)
    .then((data) => {
      // console.log(data);
      newUserInfo.setUserInfo(data);
      popupProfile.close();
...
// пустая переменная для записи идентификатора
// запрашиваемого с сервера
let userId;

// загрузка карточек с сервера
const cardsList = new Section((cardItem) => {
  const idCardCompare = userId === cardItem.owner._id;
  const idLikeCompare = cardItem.likes.some((like) => like._id === userId);
  cardsList.addItemDown(
    createCard(
      cardItem.name,
      cardItem.link,
      cardItem.likes,
